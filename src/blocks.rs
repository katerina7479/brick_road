    ...
    if let Some(id) = clicked {
        ...
        if is_double_click {
            ...
            *last_empty_click = 0.0;
            let start_day = ((world_pos.x / PIXELS_PER_DAY).max(0.0).round()) as i32;
            let duration_days = 1i32;
            let est = Estimate {
                most_likely: duration_days,
                optimistic: duration_days * 7 / 10,
                pessimistic: duration_days * 3 / 2,
                confidence: 0.8,
            };
    ...
    if mouse.pressed(MouseButton::Left) {
        if let Some(id) = resize.dragging {
            if let Some(wb) = model.work_blocks.get_mut(&id) {
                ...
                wb.duration_days = raw_dur.round() as i32;
            }
        }
        return;
    }
...
pub fn handle_block_drag(
    ...
    if mouse.pressed(MouseButton::Left) {
        if let Some((id, offset_px)) = drag.dragging {
            let new_start = (((world_pos.x - offset_px) / PIXELS_PER_DAY).max(0.0).round()) as i32;
            if let Some(wb) = model.work_blocks.get_mut(&id) {
...
pub fn sync_uncertainty_overlays(
    ...
    for (row, wb) in ordered.iter().enumerate() {
        ...
        let x_block_right = (wb.start_day + wb.duration_days) as f32 * PIXELS_PER_DAY;
        let x_pes_right = (wb.start_day + wb.estimate.pessimistic) as f32 * PIXELS_PER_DAY;
        ...
        let x_block_left = wb.start_day as f32 * PIXELS_PER_DAY;
